 ...
 * @author awang
 *
 * 根据计算过滤规则进行不同 zone 的服务调用
 *
 * 在 spring cloud 中使用 ZoneAwareLoadBalancer 为默认负载均衡器
 */
public class ZoneAwareLoadBalancer<T extends Server> extends DynamicServerListLoadBalancer<T> {
    ...
    /**
     * 选择服务
     * @param key 负载均衡器根据 key 查找 server, 默认 default
     * @return Server
     */
    @Override
    public Server chooseServer(Object key) {
        // 如果禁用 zone, 或者可用 zone 数量 <= 1, 则调用父类 BaseLoadBalancer#chooseServer 进行选择
        ...
        try {
            ...
            // 可用 zone 不为空，并且可用 zone < zoneSnapshot.size
            if (availableZones != null && availableZones.size() < zoneSnapshot.keySet().size()) {
                // 从可用 zone 中随机获取一个 zone, zone 中服务节点越多，被选中的概率越大
                String zone = ZoneAvoidanceRule.randomChooseZone(zoneSnapshot, availableZones);
                logger.debug("Zone chosen: {}", zone);
                if (zone != null) {
                    // 如果 zone 不为空，获取 zone 对应的 LoadBalancer
    ...
    BaseLoadBalancer getLoadBalancer(String zone) {
        ...
        if (loadBalancer == null) {
            ...
            // 克隆一个负载均衡算法(IRule), 默认 RoundRobinRule
            IRule rule = cloneRule(this.getRule());
            // 创建对应的 LoadBalancer，名称为 default_zone
            loadBalancer = new BaseLoadBalancer(this.getName() + "_" + zone, rule, this.getLoadBalancerStats());
            // 保存对应关系到 balancers 缓存中
